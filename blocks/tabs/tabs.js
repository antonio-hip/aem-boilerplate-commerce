...
// Create radio inputs for Tab list, handle checked through hashing in url
function generateHiddenInput(tabSectionIndex, presentTabContents, block) {
  ...
  for (let i = presentTabContents.length - 1; i > -1; i -= 1) {
    ...
    if (
      ...
      [...presentTabContents].forEach((content, index) => {
        if (index === +input.id.slice(-1)) {
    ...
    input.addEventListener('click', (e) => {
      [...presentTabContents].forEach((content, index) => {
        if (e.target.checked && index === +e.target.id.slice(-1)) {
          content.classList.add('show-content');
        } else {
...
export default async function decorate(block) {
  const presentTabContents = [
    ...block.parentElement.querySelectorAll(':scope > div.contents-wrapper > div.contents'),
  ];

  if (presentTabContents && presentTabContents.length > 0) {
    const tabSectionIndex = [...block.closest('main').children]
      .filter((section) => section.classList.contains('tabs-wrapper'))
      .indexOf(block.closest('.section'));
